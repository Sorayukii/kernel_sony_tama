...
static const unsigned int default_efficient_freq_lp[] = {0};
static const unsigned int default_efficient_freq_perf[] = {0};

/* Define default up delays for big and LITTLE cores */
static const unsigned int default_up_delay_lp[] = {0};
static const unsigned int default_up_delay_perf[] = {0};
...
static int sugov_init(struct cpufreq_policy *policy)
{
	...
	if (cpumask_test_cpu(sg_policy->policy->cpu, cpu_lp_mask)) {
		tunables->efficient_freq = kmemdup(default_efficient_freq_lp, sizeof(default_efficient_freq_lp), GFP_KERNEL);
		tunables->nefficient_freq = ARRAY_SIZE(default_efficient_freq_lp);
		tunables->up_delay = kmemdup(default_up_delay_lp, sizeof(default_up_delay_lp), GFP_KERNEL);
		tunables->nup_delay = ARRAY_SIZE(default_up_delay_lp);
	} else if (cpumask_test_cpu(sg_policy->policy->cpu, cpu_perf_mask)) {
		tunables->efficient_freq = kmemdup(default_efficient_freq_perf, sizeof(default_efficient_freq_perf), GFP_KERNEL);
		tunables->nefficient_freq = ARRAY_SIZE(default_efficient_freq_perf);
		tunables->up_delay = kmemdup(default_up_delay_perf, sizeof(default_up_delay_perf), GFP_KERNEL);
		tunables->nup_delay = ARRAY_SIZE(default_up_delay_perf);
	}

	if (!tunables->efficient_freq || !tunables->up_delay) {
